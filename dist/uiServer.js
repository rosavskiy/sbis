...
const extractWeekDescription = (name) => {
...
const parseInnKppFromFilename = (name) => {
...
const performUpload = async (params) => {
    ...
    if (normalizedClientInn.length === 10 && !effectiveClientKpp) {
        throw new HttpError(400, 'Для ИНН с 10 цифрами необходимо указать КПП (например, добавьте его в имя файла).');
    }
